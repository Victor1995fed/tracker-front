        ...
        <!--TODO: Добавить возможность создания и поиска меток прямо из формы-->
          ...
          <div class="md-layout-item md-small-size-100 md-size-33">
            <md-datepicker
              ...
              :md-open-on-focus="false"
              :md-immediately="true"
              ><label>Дата завершения</label></md-datepicker
            >
          </div>
          ...
          <div class="md-layout-item md-small-size-100 md-size-33">
            ...
            <md-field>
              <input type="range" v-model.number="form.readiness" />
              {{ form.readiness }}%
            </md-field>
          </div>
          <div
            v-if="action == this.$settings.TASK_UPDATE"
...
export default {
  ...
  methods: {
    ...
    changeText: function() {
      this.$nextTick(function() {
        let datepickerFooter = document.getElementsByClassName(
          "md-datepicker-body-footer"
        )[0];
        let buttonDatepicker = datepickerFooter.getElementsByClassName(
          "md-button-content"
        )[0];
  ...
  mounted() {
    ...
    if (parent_id !== null && parent_id !== undefined) {
      ...
      this.$http
        .get(this.$settings.API + "task/view?id=" + parent_id)
        .then(response => {
